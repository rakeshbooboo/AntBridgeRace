	...
	namespace ShaderGenerator
	{
		static class SGUILayout
		{
			...
			public static string TextField(string str, bool delayed = false)
			{
				if (delayed)
				{
					return EditorGUILayout.DelayedTextField(GUIContent.none, str, "ShurikenValue", GUILayout.MinWidth(248));
				}
				else
				{
					return EditorGUILayout.TextField(GUIContent.none, str, "ShurikenValue", GUILayout.MinWidth(248));
				}
			}
			...
			public static T DrawProObjectField<T>(T obj, params GUILayoutOption[] options) where T : UnityEngine.Object
			{
				...
				var rect = EditorGUILayout.GetControlRect(false, 13, (GUIStyle)"ShurikenValue", options);
				var btnRect = rect;
				btnRect.width = 20;
				rect.xMax -= btnRect.width;
				btnRect.x += rect.width;

				GUI.Label(rect, TCP2_GUI.TempContent(obj != null ? obj.name : "None (" + typeof(T).ToString() + ")"), (GUIStyle)"ShurikenValue");
